...
export class InterceptorService implements HttpInterceptor {
  ...
  private isInBlockedList(url: string): Boolean {
    ...
    if (url == `${environment.HOST}/accounts/Login` ||
      url == `${environment.HOST}/accounts/Logout` ||
      url == `${environment.HOST}/CollectionPoints` ||
      url == `${environment.HOST}/residents` ||
      url == `${environment.HOST}/recyclers` ||
      url == `${environment.HOST}/shops/Insert` ||
      url == `${environment.HOST}/orders`) {
      return true;
    } else {
      return false;
    }
  ...
  private addToken(req: HttpRequest<any>) {
    ...
    } else if (this.apiService.currentAccessToken == null){
        this.apiService.cargarToken();
        return req.clone({
          headers: new HttpHeaders({
            Authorization: `Bearer ${this.apiService.currentAccessToken}`
          })
